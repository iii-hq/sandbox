...
describe("MCP tools", () => {
  ...
  describe("sandbox_create schema", () => {
    ...
    it("accepts memory and network options", () => {
      const parsed = createTool.inputSchema.parse({ memory: 1024, network: true })
  ...
  describe("sandbox_exec schema", () => {
    ...
    it("parses valid input", () => {
      const parsed = execTool.inputSchema.parse({ sandboxId: "sbx_1", command: "ls" })
      expect(parsed.sandboxId).toBe("sbx_1")
      expect(parsed.command).toBe("ls")
    })

    it("accepts optional timeout", () => {
      const parsed = execTool.inputSchema.parse({ sandboxId: "sbx_1", command: "ls", timeout: 30 })
  ...
  describe("sandbox_run_code schema", () => {
    ...
    it("defaults language to python", () => {
      const parsed = runTool.inputSchema.parse({ sandboxId: "sbx_1", code: "print(1)" })
      expect(parsed.language).toBe("python")
    })

    it("accepts all supported languages", () => {
      for (const lang of ["python", "javascript", "typescript", "go", "bash"]) {
        const parsed = runTool.inputSchema.parse({ sandboxId: "sbx_1", code: "x", language: lang })
        expect(parsed.language).toBe(lang)
      }
    })

    it("rejects unsupported languages", () => {
      expect(() => runTool.inputSchema.parse({ sandboxId: "sbx_1", code: "x", language: "ruby" })).toThrow()
    })
  })
  ...
  describe("sandbox_install_package schema", () => {
    const installTool = tools.find(t => t.name === "sandbox_install_package")!
    ...
    it("defaults manager to pip", () => {
      const parsed = installTool.inputSchema.parse({ sandboxId: "sbx_1", packages: ["numpy"] })
      expect(parsed.manager).toBe("pip")
    })

    it("accepts npm manager", () => {
      const parsed = installTool.inputSchema.parse({ sandboxId: "sbx_1", packages: ["lodash"], manager: "npm" })
      expect(parsed.manager).toBe("npm")
    })

    it("accepts array of packages", () => {
      const parsed = installTool.inputSchema.parse({ sandboxId: "sbx_1", packages: ["a", "b", "c"] })
  ...
  describe("sandbox_read_file schema", () => {
    ...
    it("requires sandboxId and path", () => {
      ...
      expect(() => readTool.inputSchema.parse({ sandboxId: "sbx_1" })).toThrow()
    })

    it("parses valid input", () => {
      const parsed = readTool.inputSchema.parse({ sandboxId: "sbx_1", path: "/workspace/main.py" })
  ...
  describe("sandbox_write_file schema", () => {
    ...
    it("requires all three fields", () => {
      expect(() => writeTool.inputSchema.parse({ sandboxId: "sbx_1", path: "/a" })).toThrow()
    })

    it("parses valid input", () => {
      const parsed = writeTool.inputSchema.parse({ sandboxId: "sbx_1", path: "/a.py", content: "x=1" })
  ...
  describe("sandbox_list_files schema", () => {
    ...
    it("accepts custom path", () => {
      const parsed = listTool.inputSchema.parse({ sandboxId: "sbx_1", path: "/tmp" })
