...
describe("parseExecStream", () => {
  it("parses valid JSON chunks", async () => {
    ...
    expect(chunks[0]).toEqual({ type: "stdout", data: "hello", timestamp: 1000 })
    expect(chunks[1]).toEqual({ type: "stderr", data: "warn", timestamp: 1001 })
  ...
  it("handles non-JSON lines as stdout", async () => {
    async function* gen() {
    ...
    expect(chunks[0].data).toBe("plain text line")
  ...
  it("handles mixed JSON and plain text", async () => {
    async function* gen() {
    ...
    expect(chunks[1].data).toBe("plain text")
    expect(chunks[2].type).toBe("exit")
  })
})
