...
pub fn register(bridge: &Arc<III>, dk: &Arc<Docker>, kv: &StateKV, config: &EngineConfig) {
    // sandbox::create
    {
        ...
        bridge.register_function("sandbox::create", move |input: Value| {
            let kv = kv.clone();
            let dk = dk.clone();
            let cfg = cfg.clone();
            async move {
    ...
    {
        let kv = kv.clone();
        bridge.register_function("sandbox::get", move |input: Value| {
            let kv = kv.clone();
            async move {
    ...
    {
        let kv = kv.clone();
        bridge.register_function("sandbox::list", move |input: Value| {
            let kv = kv.clone();
            async move {
    ...
    {
        let kv = kv.clone();
        bridge.register_function("sandbox::renew", move |input: Value| {
            let kv = kv.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
                    .ok_or_else(|| iii_sdk::IIIError::Handler("id is required".into()))?;
    ...
    // sandbox::kill
    {
        let kv = kv.clone();
        let dk = dk.clone();
        bridge.register_function("sandbox::kill", move |input: Value| {
            let kv = kv.clone();
            let dk = dk.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
    ...
    // sandbox::pause
    {
        let kv = kv.clone();
        let dk = dk.clone();
        bridge.register_function("sandbox::pause", move |input: Value| {
            let kv = kv.clone();
            let dk = dk.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
    ...
    // sandbox::resume
    {
        let kv = kv.clone();
        let dk = dk.clone();
        bridge.register_function("sandbox::resume", move |input: Value| {
            let kv = kv.clone();
            let dk = dk.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
