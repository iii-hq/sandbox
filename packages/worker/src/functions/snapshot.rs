...
pub fn register(bridge: &Arc<III>, dk: &Arc<Docker>, kv: &StateKV, _config: &EngineConfig) {
    // snapshot::create
    {
        ...
        bridge.register_function("snapshot::create", move |input: Value| {
            let kv = kv.clone(); let dk = dk.clone();
            async move {
    ...
    {
        let kv = kv.clone(); let dk = dk.clone();
        bridge.register_function("snapshot::restore", move |input: Value| {
            let kv = kv.clone(); let dk = dk.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
                    .ok_or_else(|| iii_sdk::IIIError::Handler("id is required".into()))?;
    ...
    {
        let kv = kv.clone();
        bridge.register_function("snapshot::list", move |input: Value| {
            let kv = kv.clone();
            async move {
                let id = input.get("id").and_then(|v| v.as_str())
                    .ok_or_else(|| iii_sdk::IIIError::Handler("id is required".into()))?;
    ...
    {
        let kv = kv.clone(); let dk = dk.clone();
        bridge.register_function("snapshot::delete", move |input: Value| {
            let kv = kv.clone(); let dk = dk.clone();
            async move {
                let snapshot_id = input.get("snapshotId").and_then(|v| v.as_str())
                    .ok_or_else(|| iii_sdk::IIIError::Handler("snapshotId is required".into()))?;
