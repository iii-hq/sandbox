...
import { fileReadCommand, fileWriteCommand, fileUploadCommand, fileListCommand } from "./commands/file.js"
...
cli
  ...
  .option("--language <lang>", "Language (python|javascript|go|bash)")
  .action(async (sandboxId, code, opts) => {
    await runCommand(sandboxId, code, opts, getConfig())
  })

cli
  .command("list", "List active sandboxes")
  .action(async () => {
    await listCommand(getConfig())
  })

cli
  .command("kill <sandboxId>", "Kill a sandbox")
